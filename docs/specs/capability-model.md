...
- Dispatched via `runTransition(manifest, module, world, { from, to, progress }, transitionId?)`. *Implemented* for the `js` runtime in `@azphalt/runtime-reference` and `@azphalt/runtime-wasm` (QuickJS). A raw-`wasm` transition entry — two shared-memory input frames + a `progress` import — is a forward-compatible follow-up; the JS path pins the contract.
