...
`hostId` is declared rather than derived because nothing in a listing implies it. A package id and the OS-level `packageId` are already independent — they differ on the one `kind:"app"` listing in the reference registry — and `hostId` is independent of both again: it names the app extensions were *written for*, which need not be the app the listing describes. Apps get renamed, and get split in two; when that happens the id in existing `targetApps` keeps pointing at whichever app inherited the extension surface, and no rule derived from names would follow it. A directory built by guessing at that relationship quietly points users at the wrong app, and looks correct while doing it.
