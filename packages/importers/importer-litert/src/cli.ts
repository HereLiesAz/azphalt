#!/usr/bin/env node
import fs from "fs";
import { importLitert } from "./index.js";

const args = process.argv.slice(2);
if (args.length < 3) {
  console.error("Usage: azphalt-import-litert <id> <version> <author> [filepath] [--url <remoteUrl>] [--checksum <sha256>] [--size <bytes>] [--role <role>]");
  process.exit(1);
}
...
const azp = importLitert(bytes, { id, version, author, filename, remoteUrl, checksum, byteSize, role });
