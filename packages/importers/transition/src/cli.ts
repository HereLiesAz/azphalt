#!/usr/bin/env node
/** CLI for `@azphalt/importer-transition`: wrap a GL Transition into a `.azp` package. */
import { readFileSync, writeFileSync } from "node:fs";
import { basename, extname } from "node:path";
import { importGlTransition } from "./index.js";
...
const USAGE =
  "usage: azphalt-import-transition <input.glsl> --id <reverse.dns.id> " +
...
if (!args.input) fail("an input .glsl transition file is required");
if (!args.id) fail("--id <reverse-DNS id> is required (e.g. com.hereliesaz.crosswarp)");
...
const { azp, manifest, panel } = importGlTransition(source, licenseText, {
  id: args.id,
  name: args.name,
...
console.error(
  `wrote ${out}  (${manifest.id} v${manifest.version}, ${panel.controls.length} control(s), ${azp.length} bytes)`,
);
