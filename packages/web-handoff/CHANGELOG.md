...
- **web-handoff**: the storefront's own inventory of what it handed to a host.
  ...
  **The record is a guess and the API is shaped so callers cannot forget that.** `recordInstalled`
  writes `installed` on a successful handoff, which means _a host claimed the link_ — the last thing a
  ...
  `forgetInstall` deletes the entry rather than marking it `removed`: `removed` is a _host's_ statement
