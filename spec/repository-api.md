...
- `profiles` — the conformance **host profiles** the catalog targets (the same vocabulary a host declares via `@azphalt/conformance` — the blessed set is `"image"`, `"video-audio"`, `"companion"`, `"mcp"`, `"skill"`, `"script"`, `"composable"`, `"workflow"`, and `"role"`). A host reads it to confirm the registry's packages match a profile it implements (azphalt #27 items 5 & 7 — the registry side of package↔host compatibility). A registry that carries `kind:"app"` **companion** packages (`companion-app.md`) advertises `"companion"` so only a host that implements the handoff runtime browses for them; companions are almost always **app-scoped** (below), so a host also passes its own `app` id. A registry that carries `kind:"mcp"` **MCP-server** packages (`mcp-server.md`) advertises `"mcp"` so only a host that implements an MCP client browses for them. A registry that carries `kind:"skill"` **skill** packages (`skill.md`) advertises `"skill"` so only a host that implements an agent skill loader browses for them. A registry that carries `kind:"script"` **script** packages (`script.md`) advertises `"script"` so only a host that implements a native-script installer browses for them. A registry that carries `kind:"composable"` **composable** packages (`composable.md`) advertises `"composable"` so only a host that has actually linked a matching template library browses for them. A registry that carries declarative `kind:"workflow"` packages (`workflow.md`) may advertise `"workflow"`; one that carries declarative `kind:"role"` packages (`role.md`) may advertise `"role"`. Those profiles advertise host-format support only — they do not grant workflow host permissions or role authority.
...
- `kind` (string): Comma-separated list of package `kind`s to filter by — `asset` · `code` · `mixed` · `app` · `mcp` · `pack` · `skill` · `script` · `composable` · `workflow` · `role`. A host with no code sandbox passes `kind=asset`; a storefront building the host directory passes `kind=app` (see **web-handoff.md** § Host directory).
...
**Kind.** Each summary carries `kind` (`"asset"` · `"code"` · `"mixed"` · `"app"` · `"mcp"` · `"pack"` · `"skill"` · `"script"` · `"composable"` · `"workflow"` · `"role"`), so a host tells package classes apart straight from the browse list. A companion host filtering an app-scoped catalog (below) to the `companion` profile keeps the `kind:"app"` entries — no per-package detail round-trip to learn a package is a **companion** (`companion-app.md`).
...
**Workflow and role packages.** `kind:"workflow"` packages carry declarative orchestration data defined by [`workflow.md`](workflow.md); `kind:"role"` packages carry declarative host-role/persona data. Both are ordinary repository package kinds and participate in `q`, `kind`, pagination, sorting, and `app` scoping exactly like other package summaries. They do not gain access-control semantics merely because they target a particular host.
